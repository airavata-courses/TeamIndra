...
public abstract class ApplicationHandler
{
	...
	protected void readJobConfig()
	{
		...
		// Parameters to run the job from config.properties file
		jobConfig = context.getBean("jobConfig", JobConfig.class);

		context.close();

		/*
		 * System.out.println("Please enter name for the job: "); String name =
		 * scanner.nextLine(); jobConfig.setJobName(name);
		 * 
		 * System.out.println("Please enter node count(Max 72): "); int nodes =
		 * scanner.nextInt(); jobConfig.setNodes(nodes);
		 * 
		 * System.out.println("Please enter total memory: "); int memory =
		 * scanner.nextInt(); jobConfig.setMaxMemory(memory);
		 * 
		 * System.out.println("Please enter total core count: "); int cores =
		 * scanner.nextInt(); jobConfig.setCores(cores);
		 * 
		 * // flush additional input scanner.nextLine();
		 * 
		 * System.out.println("Please enter queue name: "); String qname =
		 * scanner.nextLine(); jobConfig.setQname(qname);
		 * 
		 * System.out.println("Please enter wall time in HH:MM:SS "); String
		 * wallTime = scanner.nextLine(); jobConfig.setWallTime(wallTime);
		 * 
		 * // flush additional input scanner.nextLine();
		 * 
		 ...
		 * 
		 * System.out.println("Please enter email id: "); String email =
		 * scanner.nextLine(); jobConfig.setEmail(email); }
		 */
		getInputForJob();
	}
